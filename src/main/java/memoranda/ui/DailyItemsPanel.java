...
//                // DEBUG
		...
		/*
		 * CurrentNote.addNoteListener(new NoteListener() { public void noteChange(Note
		 * note, boolean toSaveCurrentNote) { currentNoteChanged(note,
		 * toSaveCurrentNote); } });
		 */
		...
		/*
		 * AppFrame.addExitListener(new ActionListener() { public void
		 * actionPerformed(ActionEvent e) { if (editorPanel.isDocumentChanged()) {
		 * saveNote(); CurrentStorage.get().storeNoteList(CurrentProject.getNoteList(),
		 * CurrentProject.get()); } } });
		 */
	...
	void currentDateChanged(CalendarDate newdate) {
		 ...
		 */
		/*
		 * currentNoteChanged(currentNote, true); currentNote =
		 * CurrentProject.getNoteList().getNoteForDate(newdate);
		 * CurrentNote.set(currentNote, true);
		 */
		currentDate = CurrentDate.get();
	 ...
	 * 
	 * if (editorPanel.isDocumentChanged()) { if (toSaveCurrentNote) { saveNote(); }
	 * notesControlPane.refresh(); } currentNote = note;
	 * editorPanel.setDocument(currentNote); calendar.set(CurrentDate.get());
...
//		Util.debug("currentProjectChanged");
		...
		/*
		 * if (editorPanel.isDocumentChanged()) saveNote();
		 */
		/*
		 * if ((currentNote != null) && !changedByHistory && !addedToHistory)
		 * History.add(new HistoryItem(currentNote));
	...
	/*
	 * public void saveNote() { if (currentNote == null) currentNote =
	 * CurrentProject.getNoteList().createNoteForDate(currentDate);
